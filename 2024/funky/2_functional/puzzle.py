...
def generate_puzzle(topic: str, chat: ChatFn, debug: bool = False) -> PuzzleGame:
    ...
    puzzle.title = chat(TITLE_PROMPT.substitute(dict(puzzle)))
    puzzle.solution = chat(SOLUTION_PROMPT.substitute(dict(puzzle)))
    if debug:
        print(f"Solution: {puzzle.solution}")

    all_words = parse_json(chat(WORDS_PROMPT.substitute(dict(puzzle))))
    puzzle.words = filter_words(all_words, puzzle.solution)
    if debug:
        print(f"Words: {puzzle.words}")

    puzzle.clues = parse_json(chat(CLUES_PROMPT.substitute(dict(puzzle))))
    return puzzle
