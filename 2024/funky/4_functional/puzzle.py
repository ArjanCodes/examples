...
def generate_puzzle(topic: str, chat: ChatFn, debug: bool = False) -> PuzzleGame:
    ...
    def generate_str(prompt: Template) -> str:
        fn = compose(prompt.substitute, chat)
        return fn(dict(puzzle))

    def generate_json_list(prompt: Template) -> list[str]:
        fn = compose(prompt.substitute, chat, parse_json)
        return fn(dict(puzzle))

    puzzle.title = generate_str(TITLE_PROMPT)
    puzzle.solution = generate_str(SOLUTION_PROMPT)
    if debug:
        print(f"Solution: {puzzle.solution}")

    all_words = generate_json_list(WORDS_PROMPT)
    puzzle.words = filter_words(all_words, puzzle.solution)
    if debug:
        print(f"Words: {puzzle.words}")

    puzzle.clues = generate_json_list(CLUES_PROMPT)
    return puzzle
