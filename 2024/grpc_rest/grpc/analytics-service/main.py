...
class AnalyticsService(analytics_pb2_grpc.AnalyticsServiceServicer):
    ...
    def LogView(self, request, context):
        ...
        logger.info(f"Received LogView request for video_name (URL): {request.video_name}")

        session = SessionLocal()
        video_name = request.video_name

        # Try to find the video URL in the database
        analytics_entry = session.query(Analytics).filter_by(video_name=video_name).first()
