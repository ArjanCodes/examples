...
async def add_book(session: aiohttp.ClientSession, title: str, author: str) -> str:
    async with session.post('http://127.0.0.1:5000/books', data=json.dumps({'title': title, 'author': author})) as response:
        return await response.text()

async def delete_book(session: aiohttp.ClientSession, title: str) -> str:
    async with session.delete(f'http://127.0.0.1:5000/books', data=json.dumps({'title': title})) as response:
        return await response.text()
async def add_movie(session: aiohttp.ClientSession, title: str, director: str) -> str:
    async with session.post('http://127.0.0.1:5000/movies', data=json.dumps({'title': title, 'director': director})) as response:
        return await response.text()
    
async def delete_movie(session: aiohttp.ClientSession, title: str) -> str:
    async with session.delete(f'http://127.0.0.1:5000/movies', data=json.dumps({'title': title})) as response:
        return await response.text()
