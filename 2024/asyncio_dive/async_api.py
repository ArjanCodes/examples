...
class Router:
    ...
    async def handle(self, method: str, path: str, data: Optional[Dict[str, Any]] = None) -> str:
...
class AsyncAPIServer:
    def __init__(self, host: str = '127.0.0.1', port: int = 5000, router: Optional[Router] = None):
        self.host = host
        self.port = port
    ...
    def route(self, path: str, method: str = "GET") -> Callable[..., Callable[..., Awaitable[Any]]]:
...
async def add_book(data: Dict[str, Any]) -> Dict[str, str]:
    async with aiosqlite.connect('book.db') as conn:
        await conn.execute("INSERT INTO books (title, author) VALUES (?, ?)", (data['title'], data['author']))
        await conn.commit()
...
async def add_movie(data: Dict[str, Any]) -> Dict[str, str]:
    async with aiosqlite.connect('movie.db') as conn:
        await conn.execute("INSERT INTO movies (title, director) VALUES (?, ?)", (data['title'], data['director']))
        await conn.commit()
...
async def main() -> None:
    async with asyncio.TaskGroup() as tasks:
        tasks.create_task(create_table("CREATE TABLE IF NOT EXISTS books (title TEXT, author TEXT)", "book.db"))
        tasks.create_task(create_table("CREATE TABLE IF NOT EXISTS movies (title TEXT, director TEXT)", "movie.db"))
    await api.start()
